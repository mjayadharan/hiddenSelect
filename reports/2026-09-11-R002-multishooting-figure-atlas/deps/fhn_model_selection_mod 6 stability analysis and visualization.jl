...
dum_vect = Vector{Vector{Float64}}(undef, 1000)
dum_vect = [Vector{Float64}(undef, 22) for _ in 1:1000]
dum_vect = []

function dummy_testing!(dum_vect_)
    for i in 1:1000
        push!(dum_vect_, randn(22))
    end
    
end


# Finding the unstable points encountered during optimization
unstable_points = unstable_points_dict[5]
S=10
δt = Δt / S     # Time step 
fig = Figure(size = (1800, 1800)) 
window_size_range = sort(collect(keys(results_inner)))
...
ref_point = results_inner[window_size_range[end]][2]
plot_contours!(fig, fs_loss_window, center_param; ref_point_=ref_point, delta_range_=delta_range, n_grids_=num_grids,
    index_pairs_=index_pairs, figure_title_="Window size: $window_size")
fig



function odefun_new(y, p)
    F = similar(y)

    # Fill F with the same formula you have in odefun
    # R002 fix (R001-F1): the continuation lines starting with `+` were separate, discarded
    # statements, so only the linear terms survived (a linear truncation of the vector field
    # with the wrong sign of d(v')/dv). Parenthesised so the whole cubic RHS is kept.
    F[1] = (p[1 ] + p[2 ]*y[1] + p[3 ]*y[2] 
    + p[4 ]*y[1]^2 + p[5 ]*y[1]*y[2] + p[6 ]*y[2]^2 + p[7 ]*y[1]^3 + p[8 ]*y[1]^2*y[2] 
    + p[9 ]*y[1]*y[2]^2 + p[10]*y[2]^3)

    F[2] = (p[11] + p[12]*y[1] + p[13]*y[2] + p[14]*y[1]^2 
    + p[15]*y[1]*y[2] + p[16]*y[2]^2 + p[17]*y[1]^3 + p[18]*y[1]^2*y[2] 
    + p[19]*y[1]*y[2]^2 + p[20]*y[2]^3)

    return F
end

function jac_odefun(y, p)
...
function get_eigens(unstable_points_; real_only=false)
    ...
    if real_only
        return [real(eigvals(jac_)) for jac_ in jac_list_]
    else
        return [eigvals(jac_) for jac_ in jac_list_]
    end
end
