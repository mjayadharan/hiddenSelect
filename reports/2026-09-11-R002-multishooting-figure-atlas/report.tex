...
\end{longtable}
\normalsize
\noindent Animation: \texttt{figures/anim\_sweep.gif} (one frame per window size; frames in \texttt{figures/anim\_frames/}). Every figure exists as PDF and PNG under \texttt{figures/}.
...
See \texttt{REPRODUCE.md} for the pinned environment and the ordered commands (Julia stages 00--09, then the Python inventory, tables, figures, animation, gates, number manifest, and \texttt{pdflatex} twice). \texttt{report.md} is a full-content Markdown rendition of this document generated from the same sources.
\end{document}
