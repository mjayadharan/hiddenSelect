...
\subsection{The same plane without guess propagation, indexed by $K$}
...
\readthis{\textbf{These three animations are indexed by $K$, not by $\kappa$.} $K$ is the number of
shooting windows, equivalently the number of shooting nodes, $K=\lceil 100/\kappa\rceil$ on this
$N=101$ record; the nodes are the left endpoints of the windows, and every window restarts from the
datum at its node. $K$ is the quantity the manuscript's node-removal argument is stated in, which is
why these cuts carry it. Every other animation and every figure in this report is indexed by the
window size $\kappa$, and the two run in \emph{opposite} directions: the animation still plays from
$\kappa=1$ to $\kappa=100$, so $K$ counts \emph{down} from $100$ windows (a node at every sample) to
$1$ (single shooting, no interior node). Each frame prints both. The cost is the same $J$ of
\S\ref{sec:defs} throughout; it is written $J$ rather than $J_\kappa$ in these frames only, to avoid
carrying a $\kappa$ subscript on a picture labelled by $K$.}
