...
\item \textbf{3-D landscape, both arms, grazing camera} --- \texttt{anim3d\_fhn\_landscape\_wv\_ww\_low.mp4}: the slow-block plane of \texttt{anim\_fhn\_landscape\_wv\_ww\_hires.mp4} carried into 3-D at the $14^\circ$ elevation, with both the guess-propagation and the control path, \emph{without} the basin/plateau side panels or any per-frame readout, indexed by $K$ and stopped at $K=2$ ($\kappa=50$) rather than run out to single shooting.
\item \textbf{3-D landscape without guess propagation}, three camera elevations --- \texttt{anim3d\_fhn\_landscape\_wv\_ww\_control\_low.mp4} ($14^\circ$, grazing: the canyon profile and the height of the blow-up wall), \texttt{..\_control.mp4} ($32^\circ$, oblique) and \texttt{..\_control\_high.mp4} ($62^\circ$, near-overhead: the footprint of the canyon and of the plateau). Each is the slow-block surface of \texttt{anim3d\_fhn\_landscape\_wv\_ww.mp4} with the guess-propagation path removed, leaving only the minimiser the optimiser returns when every stage restarts from the same seed --- what the method does \emph{before} guess propagation is introduced. See \S\ref{sec:anim2} for the $K$ indexing these carry.
\end{itemize}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig01_problem.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig02_single_shooting.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig03_partition.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig04_multiple_shooting.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig05_node_removal.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig06_guess_propagation.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=0.75\textwidth]{figures/fig07_error_growth_schematic.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig08_lemma1_flow_sensitivity.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=0.62\textwidth]{figures/fig09_lemma2_param_sensitivity.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig10_prop1_node_removal.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig11_landscape_2d_v_v3.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig12_landscape_2d_w_wv.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig13_landscape_1d_slices.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig14_hessian_conditioning.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig15_sweep_headline.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig16_sweep_paired.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig17_sweep_coefficients.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig18_sweep_filmstrip.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig19_sweep_traces.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig20_final_coefficients.pdf}
...
\input{tables/T2_variations}
\end{table}

\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig21_schedule.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig22_penalty.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig23_noise.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig24_sparsity.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig25_optimizers.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig26_basin.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig27_iters_substeps.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig28_seedscale.pdf}
...
\input{tables/T3_solvers}
\end{table}

\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig29_solvers_eval.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig30_solvers_sweep.pdf}
...
\input{tables/T4_bugs}
\end{table}

\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig31_bug_F2.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig32_bug_F1.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig33_bug_F4.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig34_bug_ordering_shadowing.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig35_lotka_volterra.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig36_lorenz.pdf}
...
\begin{figure}[H]\centering\includegraphics[width=\textwidth]{figures/fig37_post_hessian_path.pdf}
...
\subsection{Cuts indexed by $K$, the number of windows}\label{sec:Kcuts}
Four of the slow-block animations are indexed by $K$ rather than by $\kappa$, and are the only ones in this
report that are: \texttt{anim3d\_fhn\_landscape\_wv\_ww\_low.mp4} (both arms, $14^\circ$) and the three
control cuts described next.

\readthis{\textbf{$K$ is the number of shooting windows}, equivalently the number of shooting nodes,
$K=\lceil 100/\kappa\rceil$ on this $N=101$ record; the nodes are the left endpoints of the windows, and
every window restarts from the datum at its node. $K$ is the quantity the manuscript's node-removal argument
is stated in, which is why these cuts carry it. Every other animation and every figure in this report is
indexed by the window size $\kappa$, and the two run in \emph{opposite} directions: the animations play
from small $\kappa$ to large, so $K$ counts \emph{down} from $100$ windows (a node at every sample). Each
frame prints both. Note that $K$ is \emph{not} injective on this ladder: $\kappa=50$ and $\kappa=75$ both
give $K=2$, and $\lceil 100/\kappa\rceil$ is flat wherever consecutive $\kappa$ fall in the same ceiling
bracket, so a $K$-labelled frame does not determine $\kappa$ --- which is why every frame prints the
$\kappa$ as well. The cost is the same $J$ of \S\ref{sec:defs}
throughout; it is written $J$ rather than $J_\kappa$ in these frames only, to avoid carrying a $\kappa$
subscript on a picture labelled by $K$.}

\texttt{anim3d\_fhn\_landscape\_wv\_ww\_low.mp4} is the content of the 2-D animation
\texttt{anim\_fhn\_landscape\_wv\_ww\_hires.mp4} --- the slow-block plane with both the guess-propagation
and the control path --- rendered as the same grazing-angle 3-D surface as the control cuts below, with the
2-D version's basin and plateau side panels dropped and no per-frame readout block at all: the title carries
the stage (both $K$ and $\kappa$), and nothing else competes with the surface. The basin and plateau
fractions for this plane at every stage are in Table~\ref{tab:T7} and
\texttt{landscape\_hires\_geometry.csv}; the control cuts below keep their readout.
It also \textbf{stops at $K=2$} rather than running the ladder out: the last stage shown is $\kappa=50$, the
first on the ladder whose window count reaches two. The two stages beyond it are dropped --- $\kappa=75$,
which is a second $K=2$ and would repeat the label, and $\kappa=100$, which is $K=1$, single shooting with no
interior node at all. The three control cuts below still run the full ladder to $K=1$. The colour scale and
the display clipping ceiling are computed over all sixteen stages of the grid, not only the fourteen shown,
so this cut is colour-comparable with the other \texttt{wv\_ww} animations; restricting them to the shown
stages would move the ceiling by $0.019$ decades.
At this angle the two arms separate cleanly: the guess-propagation path descends into the funnel and stays
there, while the control path stays up on the shoulder of the surface throughout.

\subsection{The same plane without guess propagation}
\texttt{anim3d\_fhn\_landscape\_wv\_ww\_control\_low.mp4}, \texttt{..\_control.mp4} and \texttt{..\_control\_high.mp4} are the slow-block 3-D animation
with the guess-propagation path deleted: the only trajectory left is the \emph{control} arm, i.e.\ the
minimiser $p^{(K)}$ the optimiser returns at each stage when every stage restarts from the same seed
vector rather than from the previous stage's answer. It is the picture of what the window ladder does
on its own, before guess propagation is introduced.

The surface tightens into a canyon as $K$ falls, exactly as in the two-arm version, but the returned
minimiser never settles into it: the projected iterate jumps across the plane from one stage to the
next, and the on-frame readout of $\lVert p^{(K)}-p^\star\rVert$ oscillates between $1.43$ and $1.95$
with no trend along the ladder, on both sides of the shared starting guess ($1.52$; below it in $6$ of
the $16$ stages) and ending at $1.55$ --- no better than the $1.44$ it already had at $K=100$.
Shortening the node set alone buys nothing: it is the \emph{carried guess} that converts the
tightening landscape into a better parameter estimate.

Three elevations are shipped because a single viewpoint hides something: at $14^\circ$ the canyon
depth and the height of the blow-up wall are legible but the footprint is foreshortened; at $62^\circ$
the footprint of the canyon and of the plateau reads like a map but the depth does not; $32^\circ$ is
the compromise. All three sweep a full turn in azimuth over their $288$ frames, so every horizontal
direction is seen in each. The per-stage $J^{\min}$ readout and the $\lVert p-p^\star\rVert$ inset of
the earlier cut are omitted from these three frames.
\readthis{The control minimiser and the minimum \emph{of the drawn plane} are different objects and
must not be read as the same marker. $\arg\min_\Pi J$ is the best point of a slice whose other $18$
coefficients are pinned at the truth; on this plane it sits on the truth star at $(0.080,-0.060)$ at
every stage of the ladder, to within one grid cell. The control minimiser is a full $20$-dimensional
optimiser output projected onto the plane, and over the ladder it sits between $0.08$ and $0.47$ away
from the truth. A renderer mode for the former exists
...
\pathtt{analysis/verify_R002.py} reads only this folder's own result files and writes \pathtt{analysis/results/gates_summary.json}. \textbf{Status at the final build: \input{tables/gates_status}.} Gates G5--G9 are bug-presence/absence gates (pre-fix present, post-fix absent); they are expected to keep passing as long as \texttt{deps/prefix/} is the \texttt{44abf4a} snapshot. All other gates are ordinary correctness or completeness gates.
\begin{center}\small
