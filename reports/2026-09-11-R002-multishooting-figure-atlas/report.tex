...
\item \textbf{3-D landscape, both arms, grazing camera} --- \texttt{anim3d\_fhn\_landscape\_wv\_ww\_low.mp4}: the slow-block plane of \texttt{anim\_fhn\_landscape\_wv\_ww\_hires.mp4} carried into 3-D at the $14^\circ$ elevation, with both the guess-propagation and the control path and \emph{without} the basin/plateau side panels of the 2-D version. Indexed by $K$.
...
indexed by the window size $\kappa$, and the two run in \emph{opposite} directions: the animations still play
from $\kappa=1$ to $\kappa=100$, so $K$ counts \emph{down} from $100$ windows (a node at every sample) to $1$
(single shooting, no interior node). Each frame prints both. The cost is the same $J$ of \S\ref{sec:defs}
...
2-D version's basin and plateau side panels dropped (both quantities are still printed per frame as numbers).
